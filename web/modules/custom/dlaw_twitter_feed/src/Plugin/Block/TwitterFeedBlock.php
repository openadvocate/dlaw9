...
namespace Drupal\dlaw_twitter_feed\Plugin\Block;
...
class TwitterFeedBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    $config = \Drupal::config('dlaw_twitter_feed.settings');
    // https://dev.twitter.com/oauth/application-only
    $api_key = rawurlencode($config->get('dlaw_twitter_feed_api_key'));
    $api_secret = rawurlencode($config->get('dlaw_twitter_feed_api_secret'));
    ...
    try {
      ...
      // Twitter API v2
      // $res = $this->httpClient->get("https://api.twitter.com/2/users/103453293/tweets?max_results=4", $options);
    }
    ...
    $renderable_tweets = [];
    $twitter_url = $this->configuration['twitter_url'];
    foreach (json_decode($res->getBody()) as $tweet_object) {
      $renderable_tweet = [
        '#theme' => 'dlaw_twitter_feed_item',
        '#tweet' => $tweet_object,
      ];
      $language = \Drupal::config('dlaw_twitter_feed.settings')->get('dlaw_twitter_feed_jquery_timeago_locale');
      $renderable_tweet['#attached']['library'][] = 'dlaw_twitter_feed/timeago';
      if ($language) {
        $renderable_tweet['#attached']['library'][] = 'dlaw_twitter_feed/timeago_' . $language;
      }
    ...
    $build['#cache']['keys'] = ['dlaw_twitter_feed', $username, "count:$num_tweets"];
